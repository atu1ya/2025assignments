...
data_path = "Tidal_Sea_Height_Data.csv"  
try:
...
# ==================== PART 2c: CONDITIONAL DISTRIBUTION OF Y(360) GIVEN DATA ====================
print("\n" + "="*60)
print("PART 2c: CONDITIONAL DISTRIBUTION OF Y(360) GIVEN DATA")
print("="*60)

# This section computes the conditional mean and variance of Y(t*) given the observed data y,
# using the multivariate normal theory for Gaussian processes.

# Frequencies (already defined above, but redefined here for clarity)
omega1 = 2 * np.pi / 12
omega2 = 2 * np.pi / 12.42
omegas = np.array([omega1, omega2])

# Retrieve estimated coefficients from Part 1b
# beta_hat[0] = sin(omega1 t), beta_hat[1] = cos(omega1 t), beta_hat[2] = sin(omega2 t), beta_hat[3] = cos(omega2 t)
b1k = beta_hat[[0, 2]]
b2k = beta_hat[[1, 3]]

# Compute amplitudes for each frequency
Ak = np.sqrt(b1k**2 + b2k**2)

# Define the autocovariance function gamma(tau)
def gamma(tau, Ak, omegas, sigma_squared):
    if tau == 0:
        return sigma_squared + np.sum(Ak)
    else:
        return np.sum(Ak * np.cos(omegas * tau))

# Build the covariance matrix Gamma (n x n) and the cross-covariance vector gamma_star (n x 1)
n = len(y)
sigma_squared = sigma2_hat
Gamma = np.zeros((n, n))
gamma_star = np.zeros((n, 1))
t_star = 360

for i in range(n):
    for j in range(n):
        Gamma[i, j] = gamma(abs(i - j), Ak, omegas, sigma_squared)
    gamma_star[i, 0] = gamma(abs(t_star - i), Ak, omegas, sigma_squared)

# Compute the conditional mean and variance using the formulas:
#   μ_{2|1} = gamma_star.T @ Gamma^{-1} @ y
#   Σ_{2|1} = gamma(0) - gamma_star.T @ Gamma^{-1} @ gamma_star
Gamma_inv = np.linalg.inv(Gamma)
mu_2_given_1 = (gamma_star.T @ Gamma_inv @ y)[0]
Sigma_2_given_1 = gamma(0, Ak, omegas, sigma_squared) - (gamma_star.T @ Gamma_inv @ gamma_star)[0, 0]

print(f"Conditional mean μ_2|1 = E[Y(360) | data] = {mu_2_given_1:.4f}")
print(f"Conditional variance Σ_2|1 = Var[Y(360) | data] = {Sigma_2_given_1:.4f}")
